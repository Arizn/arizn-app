...
class UploadController extends Controller
{
	...
	public function newFileUpload(Request $request){
        ...
        if($request->hasFile('file')) {
			...
			$ext = 'mp3';
            $tmpFileName = $tmpTimeToken.'-'.md5(uniqid()).'.'.$ext;
            ...
            $file->move(
                public_path() . '/'. $tmpFilePath. $hardPath,
                $tmpUserName.$tmpFileName
            );
            $patha = '/'.$tmpFilePath .$hardPath . $tmpUserName. $tmpFileName;
			$url = 'delete-files/?f='.urlencode(base64_encode($patha));
            return response()->json(array('filename'=>$file->getClientOriginalName(),'file'=> $patha,'initialpreview'=> $patha,'initialPreviewConfig' => [
        ['caption' => $tmpUserName. $tmpFileName, 'size' => $file->getClientSize(), 'width' => '120px', 'url' => $url],
    ],), 200);
        } else {
    ...
    protected function validator(array $inputs)
    {
	 ...
	 protected function file_validator(array $inputs)
    {

        $rules = [
    
			'file'=>'required|mimetypes:audio/mpeg,audio/mp4,audio/x-wav,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/msword,application/zip		',
        ];
